...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.maximus.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.maximus.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.maximus.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.maximus.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.maximus.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.maximus.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
