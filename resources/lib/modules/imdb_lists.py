...
class IMDBLists(object):
    ...
    def __init__(self, title_type, imdb_user=None, count=40):
        """
        Parameters:
        ...
        imdb_user:  The user id for the imdb account (numbers only). Only needed for
                    user lists.
        count:      The number of entries to show per page for pagination.
        """
    ...
    def url_decode(self, url):
        """
        Decode a URL into a dictionary of params
        """
        r = urlparse.urlparse(url)
        params = dict([ (k, v[0]) for k, v in urlparse.parse_qs(r.query).items() ])
        return r.path, params

    def get_user_lists(self, *args):
        """
    ...
    def get_imdb_url_contents(self, url, *args):
        ...
        url = "{}/{}?{}".format(self.base_url, path, self.params_encode(params))
    ...
    def get_next_link(self, path, soup):
        """
